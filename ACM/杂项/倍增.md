...
对于区间$[i,i+2^k]$求最大/最小值，实际上就是对前$[i,i+2^{k-1}]$和$[i+2^{k-1}+1,2^k]$两个区间求最大/最小值。如果我们预处理出每个数字$i$加上$2^p$次幂范围的答案，那么就可以$O(1)$得到$[i,i+2^{p+1}]$区间的结果。阈值是$O(nlogn)$的预处理。
...
我们设某一点的第$k$级父亲就是向上跳$2^k$次所到达的结点，那么对于两点求最近公共祖先就可以用$O(logn)$来解决。
